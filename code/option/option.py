...
# 模型参数有洁癖： --pre_trained_model 最好只传文件名，不要传长路径，否则 os.mkdir 会试图创建一个路径包含..的文件夹，这在操作系统里面通常是被禁止或者逻辑混乱的，所以可能会报错
if not os.path.exists(exp_dataset_dir):
    os.mkdir(exp_dataset_dir)

# only support pth, pk is Okay but the folder's name will be model.pk
